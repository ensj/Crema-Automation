...
Page.prototype.buyProduct = async function() {
    ...
    // this doesn't work
    await this.driver.actions().move(el).click();


    // these are all iframe stuff
    // id: iframeZipcode
    el = await this.findById('zboo_keyword');
    await el.sendKeys('신사동 634-5');
    el = await this.findById('zboo_search_btn');
    await el.click();
    el = await this.findByXPath("//*[contains(@onclick, '$ZclickAddress')]"); //$('#btn_search_ozipcode')
    await el.click();

    //el = await this.findById('ozipcode1');
    //await el.clear();
    //await el.sendKeys('신사동 634-5');
    //el = await this.findById('oaddr1');
    //await el.clear();
    //await el.sendKeys('서울특별시 강남구 언주로 846 (신사동)');
    //el = await this.findById('oaddr2');
    //await el.clear();
    //await el.sendKeys('글로벌메디컬빌딩 3층');
    ...
    el = await this.findByXPath("//select[@required='true']/option[last()]");
    await el.click();
