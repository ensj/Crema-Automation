 ...
 * off of the CREMA API. Will not work
 */
...
Page.prototype.getReviews = async function(loc = 0) {
    if(loc) {
        return await this.driver.get(mall.cafe24.url + locator);
    }else {
        return await this.driver.get(mall.cafe24.url + locator.reviewPath); 
    }
};
...
Page.prototype.getQuestions = async function(loc = 0) {
    if(loc) {
        ...
        return await this.driver.get(mall.cafe24.url + locator.questionPath); 
    }
};
...
Page.prototype.deleteReview = async function() {
    ...
    return await this.waitUrl(mall.cafe24.url + '/board/product/list.html?board_no=4');
};
...
Page.prototype.buyProduct = async function() {
    ...
    for(var i = 1; i < 10; i++) {
        try {
            await this.findByXPath("(//select[@required='true']/option[last()])[" + i + "]")
    ...
    await this.findByXPath("//*[@onclick=\"product_submit(2, '/exec/front/order/basket/', this)\"]")
        .then(function(el) { 
            el.click(); 
            // this does not work if crema's review popup appears
        });
    ...
    // go to basket
    // deal with "go to basket" popup that shows current content of basket
    ...
    try {
        ...
        await this.driver.executeScript("ZipcodeFinder.Opener.Event.onClickBtnPopup(" + 
                                        "{data: {zipId1: 'rzipcode1', zipId2: 'rzipcode2', " + 
                                        "addrId: 'raddr1', stateId: '', cityId: '', type: 'layer', " + 
                                        "sLanguage: 'ko_KR', addrId2: 'raddr2', form: 'frm_order_act'}})");
