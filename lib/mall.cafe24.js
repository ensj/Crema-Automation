...
class CafePage extends Page {
     ...
     *
     * @return {IThenable<T>} Resolves when the browser reaches the given url.
     */
    async login() {
     ...
     * speed up tests.
     * 
     * @param {int} code The product's code for the driver to navigate to. 
     * @return {Promise<undefined>}
     */
    async getProduct(code = 0) {
        let self = this;
        if(code) {
            ...
            return await this.check({'type':'popup'})
                    .then(function() {
     ...
     *
     * @param {string} loc The path of the questions page.
     * @return {Promise<undefined>} to be resolved after document finishes loading.
     */
    getReviews(loc = 0) {
        if(loc) {
            return this.driver.get(mall.cafe24.url + loc);
        }else {
     ...
     * into the form.
     *
     * @return {IThenable<T>} Resolves when the title of the original product page is returned.
     */
    async writeReview() {
     ...
     * 
     * @return {IThenable<T>} Resolves when the current url becomes the review board.
     */
    async deleteReview() { // make deletes return the text of the second alert.
        ...
        let res = await el.getText();
        if(res !== '게시글이 삭제되었습니다') {
            throw Error('Unexpected Alert Text: ' + res);
        }
     ...
     * in order to make any further improvements.
     *
     * @return {IThenable<T>} Resolves when the current url becomes the QnA board.
     */
     ...
     * 
     * Scrolls through all the required select selements on the page one by
     * one, checking off the last option for every one. This is because some
     * products don't reveal options for the latter select elements unless the
     * ones before it are already checked.
     * 
     * Needs a better method of error handling for the duplicate product alert.
     * 
     * @return 
     */
    async putBasket() {
     ...
     * 
     * @return {IThenable<T>} Resolves when the current url becomes the order form.
     */
    async orderAll() {
        // go to basket
        ...
        // waits until the order form is loaded.
        return await this.waitUrl(mall.cafe24.url + '/order/orderform.html');
    }
     ...
     * 
     * @return {Promise<undefined>} Resolves when the function completes.
     */
    async cancelOrder() {
     ...
     * 
     * The function starts off assuming the user is already in the order form 
     * page. It is also incredibly flaky. Currently does not work on mobile.
     *
     * @return {IThenable<T>} Resolves when the current url becomes the order result.
     */
    async buyProduct() {
        /*
         * ORDER FORM START 
        ...
        try {
            ...
            // ^ above code doesn't execute properly on certain sites, 
        ...
        if(this.type === 'mobile') {
            el = await this.findById('paymethod-new');
            await el.click();
        }
        ...
        return await this.waitUrl(mall.cafe24.url + '/order/order_result.html');
    }
}
